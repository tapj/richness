...
ggplot()+geom_point(aes(x=A1,y=A2, color=as.character(DNA_Group))) + scale_color_brewer(type="seq") + facet_wrap(~fct_relevel(Time, c("Tm7","Tm1","T40","T63","T68"))) + theme_dark()
...
pclust = pvclust(as.data.frame(prop.table(fitted(best_genus), 2)) , method.dist=dist.JSD, nboot=100, method.hclust="complete")
...
p = merge(genus_pco$li, mapping_file, by.x="row.names", by.y="X.SampleID") %>% 
  ...
  geom_point(aes(
    ...
    size=1) + facet_wrap(~DNA_Group) +
  scale_color_brewer("Microbiota type",type="qual") + 
  theme_dark() 
