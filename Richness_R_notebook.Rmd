...
mapping_file %>% filter(Time=="Tm49") %>% 
  ...
  ggplot(aes(x=Bacteroides, y=`D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Bacteroidaceae;D_5__Bacteroides`)) + geom_point() +  geom_smooth() #+ scale_y_log10() + scale_x_log10() 
...
mapping_file %>% 
  ...
  merge(., 
        ...
        by.x="X.SampleID", 
        by.y="row.names") %>% 
  ggplot() + geom_violin(aes(x=fct_relevel(Time, 
                                           c("Tm49","Tm41","Tm34","Tm20","Tm13","Tm7","Tm1","T3","T13","T17","T27","T31","T40","T47","T63","T68")), y=genus)) + facet_wrap(~Study_group) + xlab("Time") + ylab("Akkermansia") #+ theme_dark()
...
mapping_file %>% 
  ...
  merge(., 
        ...
        by.y="row.names") %>% 
  ggplot() + geom_violin(aes(x=fct_relevel(Time, 
                                           c("Tm49","Tm41","Tm34","Tm20","Tm13","Tm7","Tm1","T3","T13","T17","T27","T31","T40","T47","T63","T68")), y=genus)) + facet_wrap(~Study_group) + xlab("Time") + ylab("Ruminococcus") + scale_y_log10() #+ theme_dark()
...
ggplot()+geom_point(aes(x=A1,y=A2, color=as.character(Study_group)))  + facet_wrap(~fct_relevel(Time, c("Tm49","Tm41","Tm34","Tm20","Tm13","Tm7","Tm1","T3","T13","T17","T27","T31","T40","T47","T63","T68"))) + theme_dark() + scale_color_manual(values = Grp_color)
...
merge(genus_pco$li, mapping_file, by.x="row.names", by.y="X.SampleID") %>%
merge(., rat_metadata, by.x="Rat_ID", by.y="Animal_id_2") %>%
ggplot()+geom_point(aes(x=A3,y=A2, color=as.character(Study_group))) + facet_wrap(~fct_relevel(Time, c("Tm49","Tm41","Tm34","Tm20","Tm13","Tm7","Tm1","T3","T13","T17","T27","T31","T40","T47","T63","T68"))) + theme_dark() + scale_color_manual(values = Grp_color)
...
```{r enterotype_drivers_exploration, fig.height=12, fig.width=12}
...
p=PlotPotential2(res2$res, title = '', 
               ylab.text = 'Microbial State\n(JSD derived metric)', xlab.text = 'Chronical Perturbation strength (DSS %)', 
           cutoff = 0.5, plot.contours = TRUE, binwidth = 0.1) + theme_classic()
