...
Our aim is to develop a robust model that can link microbial diversity and low grade inflammation. We use DSS to induce colonic low grade inflammation in rats and measure bacterial diversity by sequencing the 16S rRNA genes of the fecal DNA. Diversity indexes (Shannon, Observed OTUs, beta-diversity will be computed before, during and after the DSS treatment).

## Study design

![here is the study design](Figure1_study_design.png)
...
mapping_file %>% filter(Time=="Tm49") %>% 
  ...
  merge(., t(genus[row.names(genus) %>% grep("Prevotella",.),]) , by.x="X.SampleID", by.y="row.names") %>%
  ggplot(aes(x=Bacteroides, y=`D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Bacteroidaceae;D_5__Bacteroides`+`D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Prevotellaceae;D_5__Prevotella`)) + geom_point() +  geom_smooth(method = "lm", se=FALSE) + ylab("Prevotella + Bacteroides\n(MiSeq)") + xlab("Bacteroides + Prevotella\n(qPCR)") + scale_y_log10(limits=c(0.01,1),labels = scales::percent) + scale_x_log10(limits=c(0.01,1),labels = scales::percent) 
...
mapping_file %>% filter(Time=="Tm49") %>% 
  ...
  merge(., t(genus[row.names(genus) %>% grep("Prevotella",.),]) , by.x="X.SampleID", by.y="row.names") %>%
  mutate(miseq = 
           `D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Bacteroidaceae;D_5__Bacteroides`+`D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Prevotellaceae;D_5__Prevotella`) %>% 
  with(., cor.test(Bacteroides,miseq, method="spearman"))
...
mapping_file %>% filter(Time=="Tm49") %>% 
  ...
  merge(., t(genus[row.names(genus) %>% grep("Bacteroides",.),]) , by.x="X.SampleID", by.y="row.names") %>% 
  merge(., t(genus[row.names(genus) %>% grep("Prevotella",.),]) , by.x="X.SampleID", by.y="row.names") %>% 
  .$`D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Prevotellaceae;D_5__Prevotella`
...
mapping_file %>% 
  ...
  mutate(Time=Time %>% gsub("T","",.) %>% gsub("m","-",.) %>% as.character %>% as.numeric) %>%
  filter(Time > -15) %>%
  filter(Study_group %in% c("G6","G7") ) %>%
  ggplot() + #geom_point(aes(x=Time, y=genus+10^-6)) + 
  geom_line(aes(y=genus+10^-6, 
                         x = Time, 
                         group = Rat_ID , color=Study_group)) +
  facet_wrap(~Rat_ID) + xlab("Time") + ylab("Akkermansia") + scale_y_log10() + 
  #+ theme_dark()
  #scale_color_manual("Grp",values = Grp_color) +
  geom_vline(xintercept=c(-1,13,27), col="red")
...
merge(observed_otu, mapping_file, by.x="row.names", by.y="X.SampleID") %>% 
  ...
  arrange(Rat_ID,Time) %>%
  group_by(Rat_ID) %>%
  mutate(S_obs_delta = abs(S_obs - lag(S_obs))) %>% 
  ungroup() %>%
  group_by(Study_group, Time) %>%
  summarise(S_obs_delta = median(S_obs_delta)) %>%
  
ggplot() + geom_line(aes(y=S_obs_delta, 
                         x = Time, 
#                         group = Rat_ID , color=Study_group)) + 
                                          color=Study_group)) +
  facet_wrap(~Study_group)  + 
  theme_bw() + scale_color_manual("Grp",values = Grp_color) + geom_vline(xintercept=c(-1,13,27), col="red")
...
```{r enterotype_drivers_exploration, fig.height=5, fig.width=12, include=FALSE}
...
rownames(genus_stool.int.denoized)[rev(head(order(  rowSums(abs(fitted(fit_genus[[6]], scale = TRUE) - as.vector(fitted(fit_genus[[1]], scale = TRUE))))  , decreasing = TRUE), 16))]
 ...
 f = fitted(fit_genus[[11]], scale = TRUE) - as.vector(fitted(fit_genus[[1]], scale = TRUE))
 ...
 f %>% 
  ...
  top_n(16) %>%
   ...
   ggplot() + geom_bar(aes(y=value, x=paste(Family,Genus) ), stat="identity") +
   coord_flip() + xlab("") + ylab("Genus DMM weights")
...
merge(t(genus[bistable_dominant_genera,])+10^-4, mapping_file[,c("X.SampleID", "Time", "DSS")], by.x="row.names", by.y="X.SampleID") %>%
  ...
  ggplot() + geom_histogram(aes(x=value), fill="white", bins = 20) + facet_wrap(~gsub(";","\n",gsub("D_","",variable)), scale="free") + scale_x_log10() + theme_dark() + geom_vline(aes(xintercept = Tip), col="red", size=2, alpha = 0.75) + scale_y_log10() #+ geom_density(aes(x=value))
...
hysteresis_profile_press_smooth_plot =
  ...
  geom_hline(yintercept = 10^-1.98, linetype =2) +
  #annotate("text", x = 25, y = 2*10^-1.98, label = "Tipping point") +
  geom_vline(xintercept = 0, linetype =2) + 
  scale_y_log10() +
  scale_color_manual("Press", values=c(DSS="red",Water="blue")) +
...
### microbial interaction with Bacteroides
...
mapping_file %>% 
  ...
  filter(DSS == "1" | DSS == "2" | DSS == "3") %>%
  arrange(Study_group, Rat_ID,Time) %>%
  melt(id.vars=c("Rat_ID","Time","DSS","Diet","Study_group","hysteresis_grp","D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Bacteroidaceae;D_5__Bacteroides")) %>%
  rename(Bacteroides = `D_0__Bacteria;D_1__Bacteroidetes;D_2__Bacteroidia;D_3__Bacteroidales;D_4__Bacteroidaceae;D_5__Bacteroides`) %>%
  group_by(Rat_ID,variable,hysteresis_grp) %>%
  summarise(bacteroides_cor=cor(Bacteroides,value, method="spearman")) %>%
  mutate(bacteroides_cor = ifelse(is.na(bacteroides_cor),0,bacteroides_cor)) %>%
  ungroup() -> bacteroides_cor



bacteroides_cor%>%
  group_by(hysteresis_grp,variable) %>%
  summarise(n=n(), median=median(bacteroides_cor), q25=quantile(bacteroides_cor,0.25), q75 = quantile(bacteroides_cor,0.75) ) %>%
  filter(abs(median) > 0.1) %>%
  merge(.,bacteroides_cor, by=c("hysteresis_grp","variable"), all.x=TRUE) %>%
  tidyr::separate(variable,into=c("Domain","Phylum","Class","Order","Family","Genus"),sep=";") %>%
  filter(Genus != "Other", Family != "D_5__uncultured") %>%
  ungroup() %>%
  ggplot() + geom_boxplot(aes(x=paste(Family,Genus),y=bacteroides_cor, fill=median)) + facet_wrap(~hysteresis_grp) +
  coord_flip() +
  scale_fill_continuous(low = "#FF0000", high="#00FF00") +
  ylab("Genus correlation over time with\nBacteroides rel. abundance") + xlab("")
