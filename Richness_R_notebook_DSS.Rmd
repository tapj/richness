...
genus_jsd %>%
  ...
  ggplot() + 
  geom_line(aes(x=days  , y=m, group=hysteresis_grp), col="darkgrey", size=3) +
  geom_line(aes(x=days  , y=q25, group=hysteresis_grp), col="darkgrey", linetype=2  ) +
  geom_line(aes(x=days  , y=q75, group=hysteresis_grp), col="darkgrey", linetype=2) +
 
  geom_hline(yintercept = 1-0.19, col="pink", alpha=0.5, size=3) +
  ...
  guides(col=FALSE) + ylab("Microbiota Stability\n(1- JSD by step)") + xlab("Days") -> plot_stabibility_resilience

plot_stabibility_resilience
...
rownames(genus_stool.int.denoized)[rev(head(order(  rowSums(abs(fitted(fit_genus[[9]], scale = TRUE) - as.vector(fitted(fit_genus[[1]], scale = TRUE))))  , decreasing = TRUE), 10))]
 ...
 f = DirichletMultinomial::fitted(fit_genus[[9]], scale = TRUE) - as.vector(DirichletMultinomial::fitted(fit_genus[[1]], scale = TRUE))
...
merge(t(genus[bistable_dominant_genera,])+10^-4, mapping_file[,c("X.SampleID", "Time", "DSS")], by.x="row.names", by.y="X.SampleID") %>%
  reshape2::melt(.,id.vars=c("Row.names","Time", "DSS")) %>% 
  ...
  geom_vline(aes(xintercept = Tip), col="red", size=2, alpha = 0.75) + scale_y_log10() + xlab("Genus rel. abundance") -> plot_bimodality_genera #+ geom_density(aes(x=value))

plot_bimodality_genera

ggsave("Figure_supp_bimodality_genera.pdf")
...
#bistable_dominant_genera
 ...
 f %>% 
  ...
  apply(1,sum) %>%
   ...
   tibble::rownames_to_column("genus") %>%
    dplyr::rename(value = V1 ) %>%
   mutate(`detected\nbimodality` = ifelse(genus %in% bistable_dominant_genera, "yes", "no")) %>%
    arrange(desc(value)) %>%
  top_n(10, value) %>%
   tidyr::separate(genus,into=c("Domain","Phylum","Class","Order","Family","Genus"),sep=";") %>%
   ggplot() + 
  geom_bar(aes(y=value, x=paste0("[",Family %>% gsub("D_4__","",.),"] ",Genus %>% gsub("D_5__","",.)) %>% reorder(.,value), fill=`detected\nbimodality` ), stat="identity") +
   coord_flip() + xlab("") + ylab("Genus DMM weights") -> pB_akk
...
ggraph(., layout = 'kk') + 
  ...
  scale_edge_colour_gradient2("Rho",low = "red", mid="yellow", high="green") +
  theme_dark() + theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
                       axis.ticks.x=element_blank(), axis.ticks.y=element_blank(),
...
cowplot::plot_grid(
  global_akk_net_plot,
  plot_stabibility_resilience,nrow = 2, rel_heights = c(1,2))
...
physio %>%
  ...
  merge(.,rat_metadata,by.x="Rat_ID",by.y="Animal_id_2") %>%
  select(-SUBJID,-diet,-Cage_provider,-Animal.Identification,-Filiation,-Bacteroides,-Weight) %>%
  reshape2::melt(id.vars=c("Rat_ID","dose","Study_group","Animal_id")) %>%
  na.omit() %>%
  merge(.,hysteresis, by="Rat_ID") %>%
  filter(dose > 0.9) %>%
