...
physio %>%
  ...
  melt(id.vars=c("Rat_ID","dose","Study_group","Animal_id")) %>%
  filter(variable %in% c("colon length\n(cm)","distal\nhistological score","Body weight variations\nduring the 3 DSS pulses\n(gram)")) %>%
  #filter(variable %in% c("length_colon","Total_distal","Body_weigth_dynamics\n(3 DSS pusles)")) %>%
  na.omit() %>%
  #merge(.,hysteresis, by="Rat_ID") %>%
  #filter(dose > 0.9) %>%
  ...
  facet_wrap(~variable, scale="free_y") + xlab("DSS Dose (%)") + ylab("parameter value") -> p_physio
