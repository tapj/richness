...
mloop(retention=0,n.points=100,period=99, m=1,n=3, b.y=0, b.x=15.4778485, cx = 10.9715306, cy=-0.9264833, phase.angle = -7.3910444) %>% with(., data.frame(x=.$x,y=.$y, type="resistant")),


mloop(retention=0,n.points=100,period=99, m=1,n=3, b.y=-0.6499827, b.x=16.5271817, cx = 11.8892321, cy=-1.7996530, phase.angle = -7.3336891) %>% with(., data.frame(x=.$x,y=.$y, type="resilient")),

mloop(retention=1.3664291,n.points=100,period=99, m=1,n=3, b.y=-0.5516496, b.x=15.6476289, cx = 11.1731736, cy=-1.9507013, phase.angle = -7.4588586) %>% with(., data.frame(x=.$x,y=.$y, type="no resilient"))

) %>%
  ggplot() + geom_path(aes(x=x,y=y)) + facet_wrap(~type) +
  danr::theme_dan() + 
...
akk_df %>%
  ...
  mutate(CI_inf=m-1.96*sem, CI_sup=m+1.96*sem) %>%
  #mutate(CI_inf = ifelse(CI_inf<0, 10^-4, CI_inf)) %>%
  ggplot() + 
