...
observed_hystesis_data_df %>%
 ...
 ggplot() + 
  ...
  xlab("Akkermansia\nrel. abundance") + ylab("Press perturbation") +
  guides(col=FALSE) +
  geom_path(data=model_hysteresis_df, aes(x=x,y=y))  +
  danr::theme_dan() + 
...
cowplot::plot_grid(
  pertubation_press_plot + danr::theme_dan(),
  plot_tipping_point_crosses + ylab("Akkermansia\nTipping point"),
  plot_hysteresis_retention + scale_color_discrete("detected profile"), 
  rel_widths = c(6,4,8), nrow = 1, labels="AUTO"),
plot_hysteresis_model + scale_color_manual(values=c("red","blue")), nrow=2, labels=c(NA,"D"))
